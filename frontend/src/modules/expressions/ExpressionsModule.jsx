...
import { getChapters, getKeyphrases } from '../../api'
...
export function ExpressionsModule({ onSwitchModule }) {
  ...
  const loadKeyphrases = useCallback(
    (chId, p, per, hiddenFilter) => {
      ...
      getKeyphrases(chId, p, limit, hiddenFilter)
        .then((res) => {
  ...
  useEffect(() => {
    ...
    loadKeyphrases(chapterId, page, perPage, hiddenFilter)
  }, [chapterId, page, perPage, hiddenFilter, loadKeyphrases])
  ...
  const displayItems = (() => {
    const raw =
      shuffleOrder != null
        ...
        : data.items
    return raw.filter((item) => !hiddenIdsInView.has(item.keyphrase_id))
  })()

  return (
    <div className="app">
      <div className="app-header-row">
        <h1 className="app-title">Kus Dictionary 2.0 — Выражения</h1>
        {onSwitchModule && (
      ...
      <div className="filters-row">
        <section className="block block-1">
          <label htmlFor="chapter" className="block-label">
            Раздел
          </label>
        ...
        {chapterId != null && (
          <>
            <section className="block block-2">
              <label htmlFor="perpage" className="block-label">
            ...
            <section className="block block-sentences-toggle">
              <label htmlFor="show-hidden" className="block-label">
                Скрытые карточки
              </label>
              <div className="toggle-control">
                <label htmlFor="show-hidden" className="toggle-label">
                  <input
                    id="show-hidden"
                    type="checkbox"
            ...
            <section className="block block-sentences-toggle">
              <label htmlFor="show-sentences" className="block-label">
                Предложения
              </label>
              <div className="toggle-control">
                <label htmlFor="show-sentences" className="toggle-label">
                  <input
                    id="show-sentences"
                    type="checkbox"
      ...
      <AddCardModal
        ...
        chapterId={chapterId}
        isHistoria={false}
        onCardAdded={() => loadKeyphrases(chapterId, page, perPage, hiddenFilter)}
      />
            ...
            {!loading && !error && (
              <>
                {displayItems.length === 0 ? (
                  <p className="app-muted">В этом разделе нет выражений.</p>
                ) : (
                  displayItems.map((keyphrase) => (
                    <Card
                      key={keyphrase.keyphrase_id}
                      orderNumber={keyphrase.order_in_chapter}
                      keyphrase={keyphrase}
                      ...
                      onCardUpdated={(updated) => {
                        setData((prev) => ({
                          ...
                          items: prev.items.map((it) =>
                            it.keyphrase_id === updated.keyphrase_id ? { ...it, ...updated } : it
                          ),
                        }))
                      }}
                      onCardDeleted={() => loadKeyphrases(chapterId, page, perPage, hiddenFilter)}
                    />
                  ))
                )}
              </>
